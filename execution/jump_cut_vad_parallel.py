...
def get_keyframes(input_path: str) -> list[float]:
    """Get all keyframe timestamps from a video. Results are cached."""
...
def apply_restart_cuts(speech_segments: list[tuple[float, float]],
    ...
    """Apply restart cuts: remove segment with trigger phrase + previous segment."""
...
def encode_segment_full(idx: int, input_path: str, start: float, end: float,
                        seg_path: str, encoder_args: list[str], audio_filter: str,
                        video_filter: str) -> tuple[int, str]:
    """Full re-encode a single segment."""
...
def encode_single_segment(args: tuple) -> tuple[int, str]:
    ...
    return encode_segment_full(idx, input_path, start, end, seg_path, encoder_args, audio_filter, video_filter)
...
def encode_segment_smart(idx: int, input_path: str, start: float, end: float,
    ...
    if (part1_end_kf is None or part3_start_kf is None or
        ...
        return encode_segment_full(idx, input_path, start, end, seg_path, encoder_args, audio_filter, "")

    import tempfile as tf
    with tf.TemporaryDirectory() as tmpdir:
        parts = []

        # Part 1: Re-encode from start to keyframe
        ...
        # Part 3: Re-encode from keyframe to end
        part3 = os.path.join(tmpdir, "part3.mp4")
        cmd = [
            "ffmpeg", "-y",
            "-i", input_path,
            "-ss", str(part3_start_kf),
            "-to", str(end),
        ] + encoder_args
...
def concatenate_segments_parallel(input_path: str, segments: list[tuple[float, float]], output_path: str,
    ...
    print(f"Concatenating {len(segments)} segments (parallel {workers}x, {mode})...")
    if enhance_audio:
    ...
    # Pre-fetch keyframes for smart rendering
    keyframes = None
...
def main():
    ...
    parser.add_argument("--smart-render", action="store_true", default=False,
                        help="Use smart rendering (faster but may have 1-frame glitch at boundaries)")
    ...
    # Concatenate using PARALLEL encoding
